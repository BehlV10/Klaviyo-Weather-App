...
from signup.models import WeatherSubscription
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
		...
		s = SMTP('smtp.gmail.com', 587)
			...
			s.starttls()
			s.ehlo()
			s.login('behlv10weather', 'WeatherApp')	# app-specific verification code
		...
		for e in email_dict.keys():
			try:
				...
				print('email sent to ' + str(e) + '.')
			except SMTPException as se:
				print('email to ' + str(e) + ' not sent, because:\n' + str(se))
				continue
		s.close()
